...
### 6. Gemini JSON Truncation (Root Cause of Fallback Trigger)
**Root Cause**: The reason Gemini triggered the fallback in the first place, returning `"Unterminated string starting at: line 4 column..."`, was due to a strict `max_output_tokens: 800` limit initialized for `ideas_generation`. Generating 5 distinct ideas often exceeds 600-900 tokens, causing the AI API to abruptly terminate the generation mid-JSON string.
**Resolution**: Increased the `max_output_tokens` default to `2500` in [idea_service.py](file:///Users/cortex/ventures/linkedin-as-a-service/apps/ai_engine/app/services/idea_service.py) and implemented a robust `{...}` substring extractor in [llm_service.py](file:///Users/cortex/ventures/linkedin-as-a-service/apps/ai_engine/app/services/llm_service.py) to safely strip any hallucinated markdown fences before passing to `json.loads()`. Gemini directly generates the responses now without requiring the Ollama fallback.
