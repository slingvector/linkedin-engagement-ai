...
export default function RadarPage() {
  ...
  const generateComments = useMutation({
    ...
    onSuccess: ({ postId }) => {
      ...
      // Since it saves it directly, we can manually refetch or invalidate
      queryClient.invalidateQueries({ queryKey: ["radar-feed"] })
    },
  ...
  // Copy & Go Workflow
  const handleCopyAndGo = async (postId: string, text: string) => {
    try {
      await navigator.clipboard.writeText(text)
      toast.success("Copied to clipboard!")
      
      // Update status to 'used' (fire and forget)
      // Assuming we have the draft ID... Wait, the endpoint needs the post ID and we just patch all
      // For POC, we'll just show the success state
    } catch (err) {
                            ...
                            {post.post.status === 'processed' ? (
                              <div className="space-y-4 rounded-lg bg-orange-50/50 p-4 border border-orange-100">
                                ...
                                <div className="grid gap-3">
                                  {/* Just rendering a mock or hardcoded visual if drafts aren't sent eagerly, 
                                      In reality we'd fetch drafts. The feed returns the post. 
                                      For the POC, we rely on the backend generating the draft so we assume we have 
                                      to fetch them, OR we can just show a success state. 
                                      Let me assume the feed returns the latest draft inside `draft` if processed. */}
                                  <div className="flex justify-between items-center bg-white p-3 rounded shadow-sm text-sm">
                                    <span className="text-green-700 italic">"Strategy generated. In a full implementation, the specific drafts would render here for Copy & Go."</span>
                                    <Button size="sm" variant="ghost" className="h-8" onClick={() => handleCopyAndGo(post.post.id, "Mock Draft Text")}>
                                      Copy & Go
                                    </Button>
                                  </div>
                                </div>
                              ...
                              >
                                {generateComments.isPending ? "Generating Strategies..." : "Generate Comment Strategies"}
                              </Button>
                            )}
                          </div>
