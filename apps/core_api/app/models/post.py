...
class Post(Base, UUIDMixin, TimestampMixin):
    ...
    # ── V2: Virality Scoring ──────────────────────────────────────────────────
    virality_score = Column(
        Integer,
        nullable=True,
        comment="AI-predicted virality score 0-100. Set on draft creation.",
    )
    score_breakdown = Column(
        JSONB,
        nullable=True,
        comment="Breakdown: {hook_strength, readability, value_density, cta_quality}",
    )
    hook_alternatives = Column(
        JSONB,
        nullable=True,
        comment="List of [{hook, predicted_score}] alternatives generated by AI",
    )
    actual_engagement_rate = Column(
        Integer,
        nullable=True,
